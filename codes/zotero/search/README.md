# Zotero Search Ingestion

Lightweight ingestion pipeline for literature discovery and Zotero import.
...
From `codes/zotero/search`:

```bash
make help
make query OUT=/tmp/search_query QUERY="computational psychiatry"
make seed-collection OUT=/tmp/search_seed SEED_COLLECTION_KEY=YOUR_COLLECTION_KEY
make seed-collection WITH_LLM=1 OUT=/tmp/search_seed_llm
```

Direct CLI examples:

```bash
python3 ${MYOCEAN_ROOT}/codes/zotero/search/cli.py query \
  --query "computational psychiatry" \
  --output-dir /tmp/search_query \
  --with-s2

python3 ${MYOCEAN_ROOT}/codes/zotero/search/cli.py seed \
  --seed-collection-key YOUR_COLLECTION_KEY \
  --output-dir /tmp/search_seed \
  --with-s2 \
  --with-llm-triage \
  --llm-provider openai \
  --llm-model gpt-4o-mini \
  --llm-fusion-alpha 0.6 \
  --llm-fusion-beta 0.4
```
